...
app.get('/api/:prod_name', (req, res) => {
  let prod_name = req.params.prod_name;
  db.knex.select('name', 'price', 'sale_price', 'number_of_reviews', 'average_score', 'unix_timestamp(deal_ends) as time', 'units_sold', 'shipping_option', 'drop_count').from('items').where({'name' : prod_name.toString()})
    .then(results => {
